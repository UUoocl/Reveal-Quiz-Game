...
console.log(clueElement.Type)
    ...
    switch(clueElement.Type){
        case 'Text':
            document.querySelector(`[id=question-template]`).innerHTML = `<h2 class=\"r-fit-text\">${clueElement.Clue}</h2><aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`;
            break;
        case 'Image':
        ...
        case 'Video':
            ...
            break;
        case 'Audio':
            document.querySelector(`[id=question-template]`).innerHTML = `<p class=\"r-fit-text\">${clueElement.Clue}</p><br><audio controls id=\"media\" src=\"media/${clueElement.File}\"></audio><aside class=\"notes\" id=\"temp-notes1\">template notes 📝</aside>`;
            break;
    }

    //document.querySelector(`[id=question-template]`).innerHTML = clueElement.Clue;
    document.querySelector(`[id=clue-selected]`).innerHTML = `<h2 class=\"r-fit-text\">${clueElement.Category}<br>${clueElement.Number*100}</h2><aside class="notes" id="temp-notes0">Press any key to go to the clue</aside>`;
...
//update template notes
    var navOptions1 = "<b>>(1):</b> get response, <b><(0):</b> Play media<br>, <b>+(3):</b> read aloud<br>";
    var navOptions2 = "<b>(1):</b> Correct response, <b><(0):</b> Incorrect response<br>"
    var navOptions3 = "<b>>(1):</b> Back to Game Board, <b><(0):</b> Get more responses<br>"
